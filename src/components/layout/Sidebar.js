...
export default function Sidebar({ isCollapsed = false }) {
    ...
    return (_jsxs("aside", { className: cn("flex h-full shrink-0 flex-col border-r border-[#F5E2DA] bg-white transition-all duration-300", isCollapsed ? "w-20" : "w-64"), children: [_jsx("div", { className: cn("flex h-16 shrink-0 items-center", isCollapsed ? "justify-center px-0" : "px-5"), children: isCollapsed ? (_jsx("img", { src: favicon, alt: "TeqCertify", className: "h-8 w-8 object-contain" })) : (_jsx("img", { src: logo, alt: "TeqCertify", className: "h-7 w-auto object-contain" })) }), _jsx("nav", { className: "flex-1 overflow-y-auto pl-3 pr-1.5 pb-4", children: sections.map((section, idx) => (_jsxs("div", { className: cn(idx > 0 && "mt-4"), children: [section.label && !isCollapsed && (_jsx("p", { className: "px-4 pb-2 text-[9px] font-bold uppercase tracking-wider text-[#B7A79D]", children: section.label })), _jsx("ul", { className: "space-y-0.5", children: section.items.map((item) => (_jsx("li", { children: _jsxs(NavLink, { to: item.to, end: item.to === "/", className: ({ isActive }) => cn("flex items-center rounded-xl transition-colors", isCollapsed ? "justify-center p-3" : "gap-3 px-4 py-2.5", isActive
