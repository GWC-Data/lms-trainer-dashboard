...
export default function ForgotPassword() {
  ...
  // Once submitted, browser back navigation cannot return to the email input page
  if (!isSuccess && sessionStorage.getItem("forgotPasswordSubmitted") === "true") {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (res.success) {
        sessionStorage.setItem("forgotPasswordSubmitted", "true");
        setIsSuccess(true);
      } else {
        setSubmitError(res.message || "Failed to process password reset.");
      }
    } catch (err: unknown) {
      if (err instanceof AxiosError && err.response?.data) {
        const msg = (err.response.data as { message?: string }).message;
        setSubmitError(msg || "Failed to process password reset request.");
      } else {
        ...
        >
          {isSuccess ? (
            <div>
              ...
              <h1 className="text-2xl font-bold text-[#3A2A22]">Check Your Email</h1>
              <p className="mt-2 text-sm text-[#8C7A70]">
                If <span className="font-semibold text-[#3A2A22]">{email}</span> is registered as a trainer account, a password reset link has been sent to your inbox.
              </p>
              <div className="mt-6">
                <Link
                  to="/login"
            ...
            <>
              <h1 className="text-2xl font-bold text-[#3A2A22]">Forgot Password</h1>
              <p className="mt-1 text-sm text-[#8C7A70]">Enter your registered trainer email address.</p>
              ...
              <form onSubmit={handleSubmit} noValidate className="mt-6 space-y-4">
                ...
                <div>
                  <label htmlFor="forgotEmail" className="text-xs font-medium text-[#6B5A52]">
                    Trainer Email
                  </label>
