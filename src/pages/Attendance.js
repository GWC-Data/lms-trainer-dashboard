...
import { offlineBatches, attendanceForBatch, traineesForBatch, scheduleForBatch, courseById, courses, } from "@/data/mockData";
...
export default function Attendance() {
    ...
    const coursesWithOfflineBatches = useMemo(() => courses.filter((c) => allBatches.some((b) => b.courseId === c.id)), [allBatches]);
    const [courseFilter, setCourseFilter] = useState(requestedBatch?.courseId ?? ALL);
    const filteredBatches = useMemo(() => (courseFilter === ALL ? allBatches : allBatches.filter((b) => b.courseId === courseFilter)), [allBatches, courseFilter]);
    ...
    const [rosterByBatch, setRosterByBatch] = useState(() => Object.fromEntries(allBatches.map((b) => [b.id, attendanceForBatch(b.id)])));
    ...
    const filterBar = (_jsxs("div", { className: "flex flex-col gap-3 sm:flex-row sm:items-center", children: [_jsxs("select", { value: courseFilter, onChange: (e) => handleCourseFilterChange(e.target.value), className: "h-10 rounded-xl border border-[#F0DED4] bg-white px-3 text-sm text-[#3A2A22] focus:border-[#DE896A] focus:outline-none focus:ring-2 focus:ring-[#DE896A]/20", children: [_jsx("option", { value: ALL, children: "All Courses" }), coursesWithOfflineBatches.map((c) => (_jsxs("option", { value: c.id, children: [c.name, " \u2014 ", c.level] }, c.id)))] }), _jsxs("span", { className: "text-xs text-[#B7A79D]", children: [filteredBatches.length, " offline batch", filteredBatches.length === 1 ? "" : "es"] })] }));
    if (!batch || !course) {
        return (_jsxs("div", { className: "space-y-5", children: [filterBar, _jsx(Card, { children: _jsx(CardContent, { className: "py-10 text-center text-sm text-[#B7A79D]", children: courseFilter === ALL ? "No offline batches are assigned yet." : "No offline batches for this course." }) })] }));
    }
    const [datePart, timePart] = (batch.nextSession ?? "").split(", ");
    return (_jsxs("div", { className: "space-y-5", children: [filterBar, filteredBatches.length > 1 && (_jsx("div", { className: "flex gap-3 overflow-x-auto pb-1", children: filteredBatches.map((b) => {
